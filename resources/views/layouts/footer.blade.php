<footer>
    <div class="footer__container">
        <div class="container footer_wraper">
            <div class="logo">
                <a href="#"><img src="img/logo.png" alt="logo"></a>
            </div>
            <div class="footer__content">
                <div class="footer__content_nav">
                    <a href="" class="content-menu__item">Главная</a>
                    <a href="" class="content-menu__item">Мужское</a>
                    <a href="" class="content-menu__item">Женское</a>
                    <a href="" class="content-menu__item">Для клиента</a>
                    <a href="" class="content-menu__item">Контакты</a>
                </div>
                <div class="header_cart_container">
                    <div class="header_cart">
                        <a href="#" class="menu-link cart"><img src="img/vk.png" alt="cart-icon"></a>
                    </div>
                    <div class="header_cart">
                        <a href="#" class="menu-link cart"><img src="img/Instagram.png" alt="cart-icon"></a>
                    </div>
                </div>
            </div>
        </div>

    </div>
</footer>
</body>
</html>
